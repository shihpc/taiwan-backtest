...
- **兩班都刻意排在台北午夜前**：GitHub cron 會延遲（**2026-09-08 重算的實測：中位數 2h01m／
  p90 4h44m／最大 12h23m**，四支 workflow 187 筆樣本，見該檔頂端註解——**取代 09-07 那組
  「120 筆／中位數 3h02m／最大 9h45m」，那組整組是錯的**，樣本漏抽 `taiwan-flow-live-v2/daysummary.yml`
  這個家族內最早的 slot），排 22:37 那類時間會跨午夜把目標日滾成隔天
  （taiwan-stock-news 2026-07-16 的實際事故）。**該檔原寫「常態延遲 1~2 小時」無實測支撐，
  2026-09-07 已依實測更正。**
...
- 參考時鐘上：`< 09:07`（台北 21:07 前）＝**正常**（今日班次尚未排定）；`09:07~19:07`
  （台北 21:07 ~ 隔日 07:07）＝**等待資料發布**（主班＋兜底＋實測延遲的窗內）；
  `>= 19:07` 仍缺＝**資料缺漏**。
- **紅線 19:07＝主班 21:07 +10 小時，2026-09-07 使用者裁定**（原為 13:07＝主班+4h）：**門檻值
  維持不變，但其依據的統計數字已於 2026-09-08 重算更正**——09-07 寫的「120 筆／中位數 3h02m／
  p90 6h09m／最大 9h45m／>4h 佔 26.7%／>10h 為 0 筆」**整組是錯的**（樣本漏抽
  `taiwan-flow-live-v2/daysummary.yml`，那是家族內最早的 slot）。更正後：四支 workflow
  （`taiwan-flows/daily.yml`／`postmkt/build.yml`／`taiwan-flow-live-v2/daysummary.yml`／
  `taiwan-stock-news/build-news.yml`）2026-06-16~09-07 共 **187 筆** `event:schedule` run，
  中位數 2h01m／p90 4h44m／最大 **12h23m**；>4h 12.8%、>10h **2 筆(1.1%)**、>14h 0 筆。
  **結構性事實**：GitHub 的延遲是**絕對排空時刻**、不是相對 slot 的固定倍數（08-27／28 塞車時
  四支的 `created_at` 黏在同一段絕對時刻），所以同一次事故下 slot 愈早量到的「延遲」愈大，
  **「>10h 幾筆」取決於抽了哪些 workflow**——這正是上一輪出錯的機制；兜底班在塞車事故中也
  幾乎沒有保護力（主班與兜底前後腳一起被釋出）。本 repo `walkforward.yml` **自身只有 4 筆**
  （09-03 才建檔，延遲 3h13m~5h06m），**N=4 無法自證尾端**。因此紅線的理由**不是**
  「>10h 從沒發生過」（該宣稱已被推翻），而是：三支 13:xx 班對 13:07Z 計延遲的**代理推估**
  （N=151，**推測、非本站直接觀測**）誤報率約 2%（3 筆，全落在 08-27／28 單一次事故、
  9h51m~10h07m 正好騎在線上），換取距硬期限 **4h53m** 的人工補救餘裕；對照 +4h 代理誤報
  16.6%、+8h 4.0%、+12h 0% 但餘裕只剩 2h53m。上限則受 `walkforward_daily.py` 的硬期限
  `target=(now−12h)` 約束（主班 +14h53m ＝台北隔日 12:00 後參考日滾成隔天、該日永久漏記）。
  **取樣限制**：GitHub 只保留 90 天 run 紀錄，尾端只看得到 08-27／28 這一次事故，任何
  「最大＝X」都是這次事故的函數而非平台上界；家族內尚有約 10 支帶 cron 的 workflow 未抽，
  「>10h 2 筆」是下界不是全集。
