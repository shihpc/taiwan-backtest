...
- **兩班都刻意排在台北午夜前**：GitHub cron 會延遲（**實測中位數 3h02m／p90 6h09m／最大 9h45m**，
  120 筆樣本，見該檔頂端註解），排 22:37 那類時間會跨午夜把目標日滾成隔天
...
- 參考時鐘上：`< 09:07`（台北 21:07 前）＝**正常**（今日班次尚未排定）；`09:07~19:07`
  ...
  `>= 19:07` 仍缺＝**資料缺漏**。
- **紅線 19:07＝主班 21:07 +10 小時，2026-09-07 使用者裁定**（原為 13:07＝主班+4h）：依 120 筆
  `event:schedule` run 的延遲實測（中位數 3h02m／p90 6h09m／最大 9h45m），**>4h 佔 26.7%**——
  舊門檻的常態誤報率約四分之一，2026-09-07 就誤報過一次；**>10h 為 0 筆**。上限則受
  `walkforward_daily.py` 的硬期限 `target=(now−12h)` 約束（主班 +14h53m ＝台北隔日 12:00 後
  參考日滾成隔天、該日永久漏記），19:07 距它仍有近 5 小時餘裕。
