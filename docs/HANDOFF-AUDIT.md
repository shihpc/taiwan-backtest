...
2. **規則修剪順序**：playbook 六桶中兩桶（大跌、小跌）是**看過 18 年成績之後**刪除的
   ...
   停損/停利掃描（analysis19）原稱「入選標準先寫死才跑」——**2026-09-05 稽核判定未能核實**：
   docstring 僅含策略與掃描格點說明，且腳本與結果 CSV 同在 commit 3005b2e 進庫，
   「格點預先指定」與「勝出判準預先指定」皆無事前公開證據。
...
出場（兩種）：**停損＝進場價 × 0.75%**（觸價出）；未觸損則 **13:44 收盤平**。無停利。
成本 2 點/來回。每日最多一筆單、無隔夜倉。
**2026-09-05 口徑統一裁定**：收盤平＝13:44 最後成交（≤13:44:59），不含 13:45 定盤
tick——生產腳本原用 ≤13:45:00（a16 口徑）、主張 A/B 用 a19 口徑，稽核揭露後由使用者
裁定統一 13:44 並已改 `walkforward_daily.py`；帳冊 2026-09-03/04 兩列為舊口徑不回改。

凍結時點稽核：v3（無停損版）定版＝taiwan-flow-live-v2 commit `167483f`
（2026-09-03T14:44Z＝台北 22:44）；v3-SL 切換＝本 repo commit `da32160`
（2026-09-04T09:46Z＝台北 17:46）。**2026-09-05 稽核註記**：兩個定版 commit 都在
其首個記帳交易日**收盤之後**（台北 13:45 收盤），故 2026-09-03（v3 舊帳）與
2026-09-04（v3-SL 首筆）不能算嚴格事前凍結樣本；**嚴格乾淨的前推樣本自
2026-09-07（次一交易日）起算**。ledger 記帳 commit（9a79d19, 16:49Z）晚於規則
commit，時序無矛盾。
...
| B | v3-SL 現代段 2023–2026 四個年度皆正（2023 僅 8–12 月、2026 至 8/20，非完整年） | 總 +16,506、+742/+4,300/+4,473/+6,990、週均 +107、MDD −2,044、最大單虧 −363（扣 2 點） | 同上腳本 modern 段 | `output/v18_bracket_modern.csv` |
...
- 前推對帳：workflow 每日自動 append `walkforward/ledger.csv`，網站前端即時渲染；
  累積回落超過 −2,044 點時僅於 workflow log 發 warning（**不會自動停止**後續記帳或任何交易行為，停機是人為紀律）。判讀紀律：跑滿約一季、
...
## 5.5 稽核結果增補（2026-09-05 內部稽核，詳見 audit/AUDIT-REPORT.md）

已驗證：主張 A/B/C 數字獨立引擎逐筆重現零不符；資料品質（重複/OHLC/必要時點/SOX 對齊）
無重大問題；選約無實質前視（37 個換月日舊約皆已下市，全日量最大≡掛牌最近月）；
54 點差異完整分解（暖機截頭 14 天＋短日 1 天＋13:44/13:45 收盤口徑 56 天）。

確認且修正（不影響已發布數字）：MDD 未含起始權益（修正後四序列同值）；
跳空穿損成交模型（現代 −51 點/歷史 −1,805 點＝−3%，18/18 與 4/4 不變）。

**降級**：R5 一根延遲測試顯示歷史段兩個做多桶（小漲/漲，09:00 開盤進場）的 18 年
獲利在延遲一根後由 +36,247 轉為 −16,764——TAIEX 指數開盤含未開盤成分股昨收
（停滯效應），首分鐘漂移不可交易。**「22/22 年雙時代驗證」對做多桶不成立**；
空方桶倖存（大漲空完全、跌空剩 38%）。做多桶的證據基礎退回 3 個部分年度的
現代挖掘樣本。另：兩個定版 commit 皆在首個記帳日收盤後，嚴格乾淨前推自
2026-09-07 起算。

研究（事前註冊 commit 7ccaff4）：波動度停損、門檻擾動等候選按事前判準**無一勝出**；
成本耐受度高（現代損益兩平 35.5 點/筆）；當時裁定不開影子帳冊（後於 §5.6 由使用者
更新裁定：開兩組觀察用影子帳冊，正式策略不變）。

## 5.6 外部五年驗證與影子帳冊（2026-09-06）

外部 AI 以五年（2021-09~2026-09）重抓 MTX tick 獨立驗證，報告逐字留存
`docs/FIVEYEAR-VALIDATION-CHATGPT.md`，本方交叉核對見 `audit/AUDIT-REPORT.md`
附錄 D。要點：(1) 對方在原 491 筆重現基準 +16,455 與本方稽核引擎**逐點吻合**；
(2) **2021/2022 為虧損年**（−813 部分年／−1,910），與本方 R5 延遲檢定在 TAIEX
同年段的翻負（−520／−808）三方會合——**「現代段 4/4 正年」是樣本自 2023-08 起的
起點效應，五年視角下優勢集中 2024–2026**；(3) 對方七組出場變體配對 bootstrap
區間皆含 0，未勝出。

處置（使用者裁定）：正式 v3-SL 策略與帳冊**不變**；開兩組影子對照帳冊
「1R 保本」「1R 保本＋時間停損」（規格事前註冊 `walkforward/SHADOW-SPEC.md`，
獨立 CSV：`ledger_shadow_1rbe.csv`／`ledger_shadow_1rbe_ts.csv`，2026-09-07 起
每日與正式帳冊同班產出，滿 60 出勤日才做第一次配對評估）。

## 6. 一句話總結（供檢核者對照結論）

我方主張的強度排序（2026-09-05 稽核後修訂）：**(1) 空方兩桶的跨時代結構**（大漲反轉空
經可交易性檢驗完全倖存、跌延續空部分倖存）＞ **(1b) 做多兩桶＝僅現代 3 個部分年度的
挖掘樣本**（歷史「確認」經 R5 判定為指數停滯假象）＞ **(2) v3-SL 的正期望**（同資料，含修剪決策）＞ **(3) 具體損益數字**
